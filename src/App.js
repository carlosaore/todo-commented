...
class App extends Component {
  ...
  onSubmit = (event) => {
    ...
    // You can also create a copy of the todo's and use the push method instead of using the spread operator like I do in line 39
    // let copyOfToDos = this.state.listOfToDos
    // copyOfToDos.push(todoWithCategory)
    // and then, on setState you do listOfToDos: copyOfToDos
  ...
  deleteToDo = (todo) => {
    // Non working version using only the index
    ...
    let indexOfToDo = this.state.listOfToDos.findIndex(todoItem => todoItem.todo === todo) // finds the index of the todo
    copyToDos.splice(indexOfToDo, 1) // takes the index of the todo we clicked and deletes it
    this.setState({
